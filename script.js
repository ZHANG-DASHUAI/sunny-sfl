...
// 歌单只保留 assets/audio/ 中真实存在的音频；同名歌曲只保留一个版本。
const songs = [
  createSong(
    "qing-tian",
    "晴天",
    ["放空", "回忆", "陪伴"],
    "./assets/audio/qing-tian.mp3",
  ...
  createSong("ai-ni-wu-cha", "爱你无差", ["甜一点"], "./assets/audio/ai-ni-wu-cha.mp3", "旋律温柔但不喧闹，刚好给今天加一点暖意。"),
  createSong("shou-xie-de-cong-qian", "手写的从前", ["陪伴", "回忆"], "./assets/audio/shou-xie-de-cong-qian.mp3", "适合把旧时光轻轻翻开一页，不打扰，只看看。"),
  createSong("ye-qu", "夜曲", ["emo"], "./assets/audio/ye-qu.mp3", "有些歌不用说太多，前奏响起时安静听就好。"),
  createSong("fa-ru-xue", "发如雪", ["放空"], "./assets/audio/fa-ru-xue.mp3", "画面感很足的一首，适合戴上耳机慢慢听。"),
  createSong("ban-dao-tie-he", "半岛铁盒", ["回忆"], "./assets/audio/ban-dao-tie-he.mp3", "老歌还是耐听，像翻到一件保存很好的旧物。"),
  createSong("deng-ni-xia-ke", "等你下课", ["甜一点", "陪伴"], "./assets/audio/deng-ni-xia-ke.mp3", "适合下午听，给忙碌留一段不被催促的空白。"),
  createSong("shuo-hao-bu-ku", "说好不哭", ["emo"], "./assets/audio/shuo-hao-bu-ku.mp3", "旋律很顺，适合下班路上安安静静地听。"),
  createSong("da-ben-zhong", "大笨钟", ["放空", "甜一点"], "./assets/audio/da-ben-zhong.mp3", "俏皮一点也没关系，普通的一天需要些节奏。"),
  createSong("mojito", "Mojito", ["放空", "甜一点"], "./assets/audio/mojito.mp3", "节奏轻快，适合给普通的一天换个明亮背景。"),
  createSong("cai-hong", "彩虹", ["甜一点", "陪伴", "回忆"], "./assets/audio/cai-hong.mp3", "旋律温柔，适合把今天的步子稍微放慢一点。"),
  createSong("ting-ma-ma-de-hua", "听妈妈的话", ["放空", "陪伴", "回忆"], "./assets/audio/ting-ma-ma-de-hua.mp3", "熟悉又踏实的一首，适合在有点累时听听。")
];
...
async function playCurrentSong() {
  ...
  console.log("音频相对路径：", src);
  console.log("音频完整路径：", absoluteSrc);
...
function readHelperDuration(song) {
  return new Promise((resolve, reject) => {
    ...
    const absoluteSrc = new URL(song.audio, window.location.href).href;
    const finish = () => {
    ...
    lyricsHelperAudio.src = song.audio;
...
audioPlayer.addEventListener("error", () => {
  ...
  console.error("error src:", audioPlayer.src);
  setAudioStatus("音频加载失败，请检查路径、文件名或格式。");
