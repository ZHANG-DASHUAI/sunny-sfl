...
- 暂停事件：`pause`。
- 切歌事件：`song_change`，包含手动上一首 / 下一首和自动下一首。
- 歌词页打开事件：`lyric_open`。
- 心情分类点击事件：`mood_click`。
- 访问来源：`referrer`。
...
访问位置来自 Cloudflare Pages Functions / Worker 的 `request.cf`，只能表示大概位置，不保证精准到具体地址。如果 Cloudflare 没有返回位置，后台会显示“未知”。IP 只显示脱敏形式，例如 `1.2.3.xxx`，不会在前台页面显示。访问 `/admin` 后台本身不会写入访问日志。
...
- 有点击播放、切歌、歌词打开、心情点击，或停留超过 10 秒：标记为“疑似真人”。
- 其他情况会显示“观察中”。
