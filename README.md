...
- `静听`是主功能，播放 `assets/audio/` 中的本地音频。
...
└─ assets/
   ├─ audio_original/  原始 FLAC
   └─ audio/           压缩后的 MP3
```

四个项目文件应放在同一层，`assets/audio/` 的相对位置不要改变。
...
createSong(
  ...
  "这里写一句克制、轻松的推荐语。",
  {
    ...
    instrumentalAudio: "./assets/audio/new-song_instrumental.mp3",
    singTips: [
      "不用唱完整，喜欢哪句就唱哪句。",
      "慢慢唱，不用赶。",
      "如果唱累了，就安静听一会儿。"
...
- `mood`：心情分类数组。
- `audio`：本地音频路径。
- `instrumentalAudio`：可选的本地伴奏路径，没有时填写空字符串。
- `message`：推荐语。
...
assets/audio/           转换后的 MP3
```

完成转换后，正式部署时建议让网页只引用 `assets/audio/` 里的 MP3。当前项目暂时仍引用该目录中实际存在的 FLAC，以避免请求尚未生成的 MP3。
...
1. 把自己录制或有权使用的原始 FLAC 放入 `assets/audio_original/`。
2. 单首音频建议压缩到 **3-8MB**，推荐使用 **MP3 128kbps 或 192kbps**。微信浏览器播放 50MB 左右的大文件很容易等待过久或中断。
3. 在 `script.js` 找到歌曲，把 `audio` 改成实际路径：

```js
"assets/audio/qing-tian.mp3"
```

网页当前已经引用 `assets/audio/` 中生成的英文小写 MP3。文件名和路径必须与 Netlify 上的文件完全一致。
...
- 推荐路径格式：`./assets/audio/qingtian.mp3`。
- 不要使用 Windows 本地路径、反斜杠或电脑盘符。
- `script.js` 中的路径必须和 Netlify 实际上传的文件名完全一致，包括大小写与扩展名。
...
部署时必须确认整个 `assets/audio/` 文件夹已经上传。Netlify 区分文件名大小写，中文、空格、括号以及扩展名都必须和 `script.js` 完全一致。

在浏览器开发者工具的 Console 中点击歌曲卡片或“静听这一版”，会显示：

```text
准备播放歌曲：晴天
音频相对路径：./assets/audio/qing-tian.mp3
实际请求地址：https://你的站点地址/assets/audio/...
```
...
```text
https://你的域名.netlify.app/?debug=1
```
...
- `play()` 被浏览器拦截时显示“播放被拦截了，再轻点一下试试。”
- 路径、文件名或格式失败时显示“音频加载失败，请检查路径、文件名或格式。”
...
每首歌曲都具有 `instrumentalAudio` 字段。把你有权使用的伴奏文件放到 `assets/audio/`，例如：

```text
assets/audio/qingtian_instrumental.mp3
```

然后在歌曲的可选配置中填写：

```js
instrumentalAudio: "./assets/audio/qingtian_instrumental.mp3"
```

如果该字段为空，“轻伴唱”仍会播放当前完整音频，并提示这首歌暂时没有伴奏版。项目不会调用盗版伴奏接口，也不会在线提取人声或伴奏。
...
- 页面底部的迷你播放条可随时暂停、继续或回到当前歌曲。

如果切到后台后被系统暂停，回到页面会提示“如果音乐停了，轻点一下继续听。”。页面不会擅自自动恢复，点击迷你播放条或“继续静听”即可。
...
音频文件通常较大，请留意平台的单文件和仓库容量限制。正式分享前，建议将录音压缩成体积较小的 MP3。
