...
项目有一个隐藏的访客行为追踪后台，用于站长自己查看谁打开过页面、什么时候打开、停留多久、听了什么歌、做了哪些操作。后台需要密码，普通用户看不到访问数据，也不会影响音乐播放、歌词高亮、R2 音频加载或自动下一首。
...
- 打开歌曲：`song_open`。
- 播放事件：`play`，记录 `songId`、中文歌名、模式。
- 暂停事件：`pause`。
- 歌曲播放完成：`ended`。
- 拖动进度条：`seek`。
- 上一首 / 下一首：`prev_song`、`next_song`。
- 切歌事件：`song_change`。
- 切换静听 / 轻唱 / 伴奏：`mode_change`。
- 歌词页打开事件：`lyric_open`。
- 浏览歌词：`lyric_scroll`。
- 心情分类点击事件：`mood_click`。
- 打开留言小纸条：`note_open`。
...
- 浏览器和系统。
- 是否微信环境。
- 访问位置：国家、城市、地区、时区、Cloudflare 机房和脱敏 IP。
- 最后访问时间。
- 每首歌累计播放次数。
...
- 只打开页面、停留少于 3 秒、没有任何点击或播放行为：标记为“疑似机器人/预加载”。
- UserAgent 包含 `Linux`、`Headless`、`bot`、`crawler`、`spider`、`preview`：标记为“疑似机器人”。
- 有点击播放、切歌、歌词打开、心情点击、歌词滚动，或停留超过 10 秒：标记为“疑似真人”。
