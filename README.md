...
- 优先播放歌曲的 `instrumentalAudio`。
- 有伴奏版时默认使用 70% 音量。
- 没有伴奏版时不会报错，会回退到 `audio`，并自动降低到 25% 音量。
...
如果该字段为空，“轻伴唱”仍会低音量播放当前完整音频，并提示这首歌暂时没有伴奏版。项目不会调用盗版伴奏接口、下载 QQ 音乐音频，也不会使用浏览器做复杂的人声分离。需要更干净的伴奏时，请先用专业工具制作伴奏版，再上传到 Cloudflare R2。
